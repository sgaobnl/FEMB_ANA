...
Last modified: Tue Sep 18 13:30:35 2018
"""
import matplotlib
matplotlib.use('Agg')
...
rdfile = resultpath +  APA + "_APA" + str(APAno) + '_gain' + str(gain) + "_tp" + str(tp) + '_results.bin'
if os.path.isfile(rdfile):
    import pickle
    with open (rdfile, 'rb') as fp:
        print rdfile
...
csvfile = resultpath +  APA + "_APA" + str(APAno) + '_gain' + str(gain) + "_tp" + str(tp) + '_results.csv'
print csvfile

with open (csvfile, 'wb') as fp:
    fp.write(",".join(str(i) for i in item_n) + "," + "\n")
    for x in all_chn_results:
