...
Last modified: 10/24/2017 10:54:58 PM
"""
...
def histogram_pedstals ( ax, chn, chndata, tp):
    clor = plot_color(tp)
    ax.hist(chndata[2][3], normed=1, color = clor)
...
def ax1_plot(ax1, one_chn_tp, chn, tp_np, wire_type, fs = 2000000, yrange=[000,4100], stuck_type_np=["Small","Small","Small","Small"]):
    ...
    for i in range(4):
        ...
        tmp_Nsps = len(one_chn_tp[i][2][3])
        if (  tmp_Nsps > Nsps ):
            y_np = one_chn_tp[i][2][3][0:Nsps]
            x_np = np.arange(Nsps)
        else:
            x_np = np.arange(tmp_Nsps)
            y_np = one_chn_tp[i][2][3][0:tmp_Nsps]
...
def ax4_plot_pre(ax4, one_chn_tp, chn, tp_np, wire_type, fs = 2000000, psd = True, avg_cycle=100):
    patch = []
    label = []
    rms_np = []
    for i in range(4):
    ...
    ax4.set_title( "Pre-coherent filter, FFT specturms of Chn%d"%(chn))

def ax4_plot_coh(ax4, one_chn_tp, chn, tp_np, wire_type, fs = 2000000, psd = True, avg_cycle=100):
    patch = []
    label = []
    rms_np = []
    for i in range(4):
    ...
    ax4.set_title( "Coherent noise, FFT specturms of Chn%d"%(chn))

def ax4_plot_post(ax4, one_chn_tp, chn, tp_np, wire_type, fs = 2000000, psd = True, avg_cycle=100):
    patch = []
    label = []
    rms_np = []
    for i in range(4):
...
#        label.append("%s wire, Chn%d, %.1f$\mu$s"%(wire_type, chn, tp_np[i]))
    ...
    ax4.set_title( "Post-coherent filter, FFT specturms of Chn%d"%(chn))
...
def ax5_plot_pre(plt, one_chn_tp, chn, tp_np, wire_type, yrange=[0,3000]):
    enc_np = []
    mker = ['o','*','^','x','d','+','s','8',"1","2"]
    for i in range(4):
        x = one_chn_tp[i][1][1]
        ...
        enc = one_chn_tp[i][2][5] * fit_result[0] 
        enc_np.append(enc)
        clor = plot_color(tp_np[i])
...
def ax5_plot_post(plt, one_chn_tp, chn, tp_np, wire_type, yrange=[0,3000]):
    enc_np = []
    mker = ['o','*','^','x','d','+','s','8',"1","2"]
    for i in range(4):
        x = one_chn_tp[i][1][1]
        ...
        fit_result = ax5_linear_fit(chn, x, y, tp_np[i])
        #enc = one_chn_tp[i][0][6] * fit_result[0] 
        enc = one_chn_tp[i][2][6] * fit_result[0] 
        enc_np.append(enc)
        clor = plot_color(tp_np[i])
        plt.scatter([tp_np[i]], [enc], color = clor, marker = mker[i] )
        plt.text(tp_np[i], 1500, "%d"%int(enc), fontsize=10)
...
def one_chn_plot_coh_filter (all_chn_results, chn,  pp, step,env="RT", femb = 0, psd = True, gain =3, DACvalue=[1,2,3,4]):
    one_chn_tp = []
    stuck_type_np = []
    ...
    ax4_plot_pre (ax2, one_chn_tp, chn, tp_np, wire_type, fs = 2000000, psd = True, avg_cycle=100)
    ax4_plot_coh (ax3, one_chn_tp, chn, tp_np, wire_type, fs = 2000000, psd = True, avg_cycle=100)
    ax4_plot_post(ax4, one_chn_tp, chn, tp_np, wire_type, fs = 2000000, psd = True, avg_cycle=100)
...
#    ax3_plot(ax3, one_chn_tp, chn, tp_np, wire_type, DACvalue) 
    ...
    plt.axes([0.77, 0.77, .18, .12])
    ax5_plot_pre (plt, one_chn_tp, chn, tp_np, wire_type, yrange=[0,3000])
    plt.axes([0.77, 0.32, .18, .12])
    ax5_plot_post(plt, one_chn_tp, chn, tp_np, wire_type, yrange=[0,3000])
