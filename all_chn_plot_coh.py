...
Last modified: Mon Jan 22 22:54:06 2018
"""
...
from paras_tp_allchn_coh import paras_tp_allchn_coh
from enc_allchn_coh import enc_allchn_coh
from enc_allchn_coh import sf_enc_allchn_coh
...
def allpedplot(ax, all_chn_results, coh_chn_np, tp_np, paras=5, wire_type="All", yrange =[000,4095], ylabel = "ADC output /bin", title = "Pedestal Measurement" ):
    paras_tp_chn = paras_tp_allchn_coh(all_chn_results, coh_chn_np, paras=paras, wire_type=wire_type)
    patch = []
    label = []
...
def allencplot(ax, all_chn_results, coh_chn_np, tp_np, paras=6, wire_type="All",  yrange =[000,3000], ylabel = "Electrons / e-", title = "ENC Measurement" ):
    rms_tp_chn = paras_tp_allchn_coh(all_chn_results, coh_chn_np, paras=6, wire_type=wire_type )
    gain_tp_chn = paras_tp_allchn_coh(all_chn_results, coh_chn_np, paras=9, wire_type=wire_type )
...
def allencplot_stuckfree(ax, all_chn_results, coh_chn_np, tp_np, paras=6, wire_type="All",  yrange =[000,3000],ylabel = "Electrons / e-", title = "ENC Measurement" ):
    ...
    for tp_no in [0,1,2,3]:
        ...
        #chn_enc_np = enc_allchn_coh(all_chn_results, coh_chn_np, tp_no, wire_type=wire_type )
        #chn_np = []
        #enc_np = []
        ...
        sf_chn_enc_np = sf_enc_allchn_coh(all_chn_results, coh_chn_np, tp_no, wire_type=wire_type )
        sf_chn_np = []
        sf_enc_np = []
...
def allfft_05_plot(ax, all_chn_results, coh_chn_np, tp_np, wire_type="All", psd=True, SF = True ):
    f_tp_chn = paras_tp_allchn_coh(all_chn_results, coh_chn_np, paras=3, wire_type=wire_type)
    p_tp_chn = paras_tp_allchn_coh(all_chn_results, coh_chn_np, paras=4, wire_type=wire_type)
    s_tp_chn = paras_tp_allchn_coh(all_chn_results, coh_chn_np, paras=11, wire_type=wire_type)
...
def all_chn_plot_coh (save_encfile, all_chn_results, coh_chn_np,  pp, step,env="RT", femb = 0, psd = True, gain =3):
#################################################
...
#    all_tp_enc_plot(ax7_3, enc_np_V  , yrange =[000,3000])
    fig7.suptitle("Post coherent noise filter, %s: FEMB%d, Pedestal, Gain, ENC"%(step, femb), fontsize = 20)
    plt.tight_layout( rect=[0, 0.05, 1, 0.95])
    plt.savefig(pp, format='pdf')
    plt.close()
