    ...
    ##rms pedestal        
        ...
        for fembloc in range(5):
            ...
            if len(ped_np)!= 0:
                ...
                if (fembloc < 10 ):
                    ax.text (chn_np[0], 19, "B" + format(APAno, "1d") + format(fembloc+1, "02d"), color = 'b' )
                else:
                    ax.text (chn_np[0], 19, "A" + format(APAno, "1d") + format(fembloc+1, "02d"), color = 'b' )
                ax.text (chn_np[0]+2, 18, "Alive", color = 'g' )
                ax.text (chn_np[0]+2, 17, "WIB%d"%(chnwib_np[0]+1 ), color = 'g'  )
                ax.text (chn_np[0]+2, 16, "FEMB%d"%(chnfemb_np[0]), color = 'g'  )
                ax.vlines(chn_np[0], 0, 20, color='m', linestyles="dotted", linewidth=0.8)
