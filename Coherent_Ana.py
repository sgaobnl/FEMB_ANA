...
for gr in grs:
    ...
    if (gr[0][0] == "U") and (gr[10]==True):
        Uinds.append(gr)
    if (gr[0][0] == "V") and (gr[10]==True):
        Vinds.append(gr)
 
print len(cols), len(Uinds), len(Vinds)

def rms_w ( chngrs):
    prerms = []
    postrms =[]
    for i in range(len(chngrs)):
        prerms.append(chngrs[i][11])
        postrms.append(chngrs[i][13])
    prerms_m =  int(np.mean(prerms)  )
    prerms_s =  int(np.std(prerms)   )
    postrms_m = int(np.mean(postrms) )
    postrms_s = int(np.std(postrms)  )
    return prerms_m, prerms_s, postrms_m, postrms_s
